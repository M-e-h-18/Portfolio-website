...
import React, { useRef, useEffect } from 'react';
    const CanvasNameDisplay2 = ({ delay = 0 }) => {
    ...
    useEffect(() => {
      ...
      const draw = () => {
        ...
        const rectY = 0; // Start from the top of the canvas
        ...
        if (rectGrowing.current) {
          rectHeightRef.current += 4; // Speed of rectangle expansion
          ballRef.current.ballRadius = rectWidth / 1.3; // Update ball radius
          ballRef.current.y = rectHeightRef.current +ballRef.current.ballRadius-10; // Adjust ball position
  
          if (rectHeightRef.current >= height) {
            rectHeightRef.current = height;
            ballRef.current.y = rectHeightRef.current + ballRef.current.ballRadius; // Final position
            rectGrowing.current = false;
          }
        }  else if (ballRef.current.moveUp) {
        // Ball move up and down logic when hovered
        const ballTop = height+18; // Ball top position
        const ballBottom = height- rectHeightRef.current + ballRef.current.ballRadius; // Ball bottom position (bottom of the canvas)
        ...
        // Reset ball position to the bottom of the rectangle when not hovered
        ballRef.current.y = height-20;
      }
    };
    ...
    const handleMouseLeave = () => {
      ...
      ballRef.current.y = height - rectHeightRef.current + ballRef.current.ballRadius; // Move ball to the bottom of the rectangle
    };
...
export default CanvasNameDisplay2;
